...
from .models import Transformer, Pooling, Dense, CoPEAttention
...
class SentenceTransformer(nn.Sequential):
    ...
    def encode(self, sentences: Union[str, List[str]],
        ...
        for start_index in trange(0, len(sentences), batch_size, desc="Batches", disable=not show_progress_bar):
            ...
            sentences_batch = sentences_sorted[start_index:end]
    ...
    def smart_batching_collate_embedding_updata(self, batch):
        num_texts = len(batch[0].texts)
        texts = [[] for _ in range(num_texts)]
        embedding = []
        ...
        desc_features = []
        tokenized = self.tokenize(embedding)
        desc_features.append(tokenized)

        # return sentence_features, embedd
        return sentence_features, desc_features
    ...
    def fit(self,
            ...
            optimizer_params: Dict[str, object] = {'lr': 8e-5},
        ...
        # Use smart batching
        if save_f:
            dataloaders[0].collate_fn = self.smart_batching_collate_embedding_updata
        ...
        for epoch in trange(epochs, desc="Epoch", disable=not show_progress_bar):
            ...
            for _ in trange(steps_per_epoch, desc="Iteration", smoothing=0.05, disable=not show_progress_bar,
                ...
                for train_idx in range(num_train_objectives):
                    ...
                    if save_f and train_idx == 0:
                        labels = list(map(lambda batch: batch_to_device(batch, self._target_device), labels))
                    else:
                        ...
                        with autocast():
                            if save_f:
                                if train_idx == 0:
                                    ...
                                    all_loss_cl.append(loss_value.item())
                                else:
                                    loss_value = loss_model(features, labels)
                                    all_loss_ab.append(loss_value.item())
                            else:
                                loss_value = loss_model(features, labels)
            ...
            self._eval_during_training(evaluator, output_path, save_best_model, epoch, -1, callback, loss_model)
            self._eval_during_training2(evaluator2, output_path, save_best_model, epoch, -1, callback)

            if save_f:
                logging.info("Contrastive Loss Mean: {}".format(np.mean(all_loss_cl)))
                if len(dataloaders) == 2:
                    logging.info("Abbreviation Loss Mean: {}".format(np.mean(all_loss_ab)))
    ...
    def _eval_during_training(self, evaluator, output_path, save_best_model, epoch, steps, callback, loss_model):
        ...
        if evaluator is not None:
            ...
            if score >= self.best_score:
                ...
                logger.info(f'now!bast_epoch/best_score={epoch}/{self.best_score}')
                if save_best_model:
                    self.save(output_path)
                    classifier_path = os.path.join(output_path, "classifier_best.pt")
                    torch.save(loss_model.classifier.state_dict(), classifier_path)
    ...
    def _eval_during_training2(self, evaluator, output_path, save_best_model, epoch, steps, callback):
        ...
        if output_path is not None:
            os.makedirs(output_path, exist_ok=True)
            eval_path = os.path.join(output_path, "test")
            os.makedirs(eval_path, exist_ok=True)
    ...
    def _load_auto_model(self, model_name_or_path):
        ...
        # pooling_model = Pooling(transformer_model.get_word_embedding_dimension(), 'weightedmean')
