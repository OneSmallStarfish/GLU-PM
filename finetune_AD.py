...
def init_logging(LOGGER):
    ...
    file_handler = logging.FileHandler('./output/pretrain_task/AD_bgl.log')
...
def parse_args():
    ...
    args.add_argument("-train_data", "--train_data", type=str,
                      default="./datasets/tasks/AD/bgl_train.json", help="train dataset")
    args.add_argument("-dev_data", "--dev_data", type=str,
                      default="./datasets/tasks/AD/bgl_dev.json", help="dev dataset")
    args.add_argument("-test_data", "--test_data", type=str,
                      default="./datasets/tasks/AD/bgl_test.json", help="test dataset")
...
def evaluate(args):
    ...
    # loss
    train_loss = losses.SingleSoftmaxLoss(model=model, sentence_embedding_dimension=model.get_sentence_embedding_dimension(), num_labels=2)

    dev_evaluator = LabelAccuracyEvaluator(dev_dataloader, softmax_model=train_loss, name='AD_dev')
    test_evaluator = LabelAccuracyEvaluator(test_dataloader, softmax_model=train_loss, name='AD_test')
