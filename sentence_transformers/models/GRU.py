...
class GRU(nn.Module):
    """
    Bidirectional GRU running over word embeddings.
    bidirectional=TRUE,是双向
    """
    def __init__(self, word_embedding_dimension: int, hidden_dim: int, num_layers: int = 1, dropout: float = 0, bidirectional: bool = False):
        ...
        # Change LSTM to GRU
        self.encoder = nn.GRU(word_embedding_dimension, hidden_dim, num_layers=num_layers, dropout=dropout, bidirectional=bidirectional, batch_first=True)

    def forward(self, features):
        token_embeddings = features['token_embeddings']
        sentence_lengths = torch.clamp(features['sentence_lengths'], min=1)
        sentence_lengths = sentence_lengths.cpu().long()
    ...
    def save(self, output_path: str):
        with open(os.path.join(output_path, 'gru_config.json'), 'w') as fOut:
    ...
    def load(input_path: str):
        with open(os.path.join(input_path, 'gru_config.json'), 'r') as fIn:
